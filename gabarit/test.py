...
#
#  (C) C̩line ANSQUER, Eric BOUCHARE, Vincent CHOQUEUSE
#
...
class TF():
    ...
    def step(self,T=None,show=True,filename=None,**options):
        ...
        if show:
            ...
            plt.xlabel("temps [s]")
            plt.ylabel("R̩ponse Indicielle")
            plt.grid()
        if filename:
    ...
    def impulse(self,T=None,show=True,filename=None,**options):
        ...
        if show:
            plt.plot(t,s,**options)
            plt.xlabel("temps [s]")
            plt.ylabel("R̩ponse Impulsionnelle")
            plt.grid()
        if filename:
    ...
    def output(self,input,T,show=True,filename=None,**options):
        ...
        if show:
            plt.plot(t,s,**options)
            plt.xlabel("temps [s]")
            plt.ylabel("R̩ponse Temporelle")
            plt.grid()
        if filename:
            self.save_csv(filename,[t,s],type="IM")
        return t,s
    ...
    def phase(self,w=None,show=True,filename=None,**options):
        ...
        if show:
            ...
            plt.xlabel("f [Hz]")
            plt.ylabel("argument = d̩phasage ")
            plt.grid()
        if filename:
...
def draw_prototype(xc,yc,xs,ys,xaxes_type,yaxes_type,filter_type="lowpass"):
	...
	if xaxes_type == "w":
		...
		plt.gca().set_xlabel('fr̩quence en Hz')
	...
	if yaxes_type == "T":
		...
		ymax=y0*10
		plt.yscale('log')
		plt.gca().set_ylabel('amplification sans unit̩')
...
def graph_poles(wc):
	...
	plt.axis('scaled')
	plt.title("diagramme des p̫les normalis̩s pour les 4 aproximations")
	plt.xlabel("Partie R̩elle /wc");
